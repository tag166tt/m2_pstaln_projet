...
def mweval_sent(sent, ggroups, pgroups, gmwetypes, pmwetypes, stats, indata=None):
    ...
    stats['CrossGapPNumer'] += sum(
        (1 if b - a > 1 else 0) for a, b in plinks1 if any(a in grp and b in grp for grp in ggroups1))
    ...
    stats['CrossGapRNumer'] += sum(
        (1 if b - a > 1 else 0) for a, b in glinks1 if any(a in grp and b in grp for grp in pgroups1))
...
if __name__ == '__main__':
    ...
    for sentId, gdata in readsents(fileinput.input(goldFP)):
        ...
        for predF, stats, gmwetypes, pmwetypes, sststats, conf in zip(predFs, statsCs, gmwetypesCs, pmwetypesCs,
            ...
            stats['nFullTagCorrect'] += sum(
                1 for k in range(len(words)) if gtags_mwe[k] == ptags_mwe[k] and glbls.get(k) == plbls.get(k))
            if printSents:
    ...
    sysprefixes = [('SYS{:0' + str(len(str(len(predFs)))) + '}  ').format(i + 1) if len(predFs) > 1 else '' for i in
                   range(len(predFs))]
    ...
    for stats, conf, pmwetypes, sysprefix in zip(statsCs, confCs, pmwetypesCs, sysprefixes):
        ...
        stats['Tag_Acc_non-Oo_in-gap'] = Ratio(stats['gold_pred_non-Oo_in-or-out-of-gap_match'],
        ...
        stats['CrossGapP'] = stats['CrossGapPNumer'] / stats['CrossGapPDenom'] if stats[
                                                                                      'CrossGapPDenom'] > 0 else float(
            'nan')
        stats['CrossGapR'] = stats['CrossGapRNumer'] / stats['CrossGapRDenom'] if stats[
                                                                                      'CrossGapRDenom'] > 0 else float(
            'nan')
        ...
        parts = [(' {1}{0:.2%}'.format(float(stats[x]), relativeColor(stats[x], statsCs[0][x])) + Colors.PLAINTEXT,
                  ...
                  '{:>7}'.format(
                      '' if x.endswith('F') or isinstance(stats[x], (float, int)) else stats[x].denominatorS)) for x in
                 ('P', 'R', 'F', 'EP', 'ER', 'EF', 'Acc',
    ...
    if len(predFs) == 1:
        ...
        for d, d2 in (('n.', 'v.'), ('v.', 'n.')):
            ...
            if not conf:
                print(
                    Colors.RED + 'No gold or predicted supersenses found: check that the input is in the right format. Exiting.' + Colors.RED + Colors.ENDC)
                sys.exit(1)
            ...
            for i, g in enumerate(lbls):
                ...
                for j, p in enumerate(lbls):
                    ...
                    colr = SPECTRUM[
                        int((v - 1) / nondiag_max * len(SPECTRUM))] if v > 0 and i != j else Colors.PLAINTEXT
    ...
    for sststats, sysprefix in zip(sststatsCs, sysprefixes):
        parts = [(' {1}{0:.2%}'.format(float(sststats['Exact Tag']['Acc']), relativeColor(sststats['Exact Tag']['Acc'],
                                                                                          sststatsCs[0]['Exact Tag'][
                                                                                              'Acc'])) + Colors.PLAINTEXT,
                  '{:>7}'.format(sststats['Exact Tag']['Acc'].numeratorS),
                  '{:>7}'.format(sststats['Exact Tag']['Acc'].denominatorS))]
        parts += [(' {1}{0:.2%}'.format(float(sststats[None][x]),
                                        relativeColor(sststats[None][x], sststatsCs[0][None][x])) + Colors.PLAINTEXT,
                   '{:>7}'.format(sststats[None][x].numeratorS),
                   '{:>7}'.format(sststats[None][x].denominatorS)) for x in ('P', 'R')]
        parts += [(' {1}{0:.2%}  '.format(float(sststats[None]['F']), relativeColor(sststats[None]['F'],
                                                                                    sststatsCs[0][None][
                                                                                        'F'])) + Colors.PLAINTEXT,
                   '         ',
                   '         ')]
        parts += [(' {1}{0:.2%}'.format(float(sststats[y]['R']),
                                        relativeColor(sststats[y]['R'], sststatsCs[0][y]['R'])) + Colors.PLAINTEXT,
    ...
    for sststats, sysprefix in zip(sststatsCs, sysprefixes):
        ...
        parts = [(' {1}{0:.2%}'.format(float(cstats[x]), relativeColor(cstats[x], cstatsBL[x])) + Colors.PLAINTEXT,
                  '{:>7}'.format(
                      '' if x.endswith('F') or isinstance(cstats[x], (float, int)) else cstats[x].numeratorS),
                  '{:>7}'.format(
                      '' if x.endswith('F') or isinstance(cstats[x], (float, int)) else cstats[x].denominatorS)) for x
                 in ('Acc', 'P', 'R', 'F')]
        for j, pp in enumerate(zip(*parts)):
            print((sysprefix if j == 0 else syspad) + ' '.join(pp))
    ...
    if len(predFs) == 1:
        ...
        print('==============')
        print(re.sub(r'=([^=]+)$', '=' + Colors.YELLOW + r'\1' + Colors.PLAINTEXT,
                     'MWEs: P={stats[P]} R={stats[R]} F={f:.2%}'.format(stats=stats, f=float(stats['F']))))
        print(re.sub(r'=([^=]+)$', '=' + Colors.PINK + r'\1' + Colors.PLAINTEXT,
                     'Supersenses: P={stats[P]} R={stats[R]} F={f:.2%}'.format(stats=sststats[None],
                                                                               f=float(sststats[None]['F']))))
        print(re.sub(r'=([^=]+)$', '=' + Colors.GREEN + r'\1' + Colors.PLAINTEXT,
                     'Combined: Acc={stats[Acc]} P={stats[P]} R={stats[R]} F={f:.2%}'.format(stats=cstats,
                                                                                             f=float(cstats['F']))))
