...
def process_sentence(words, lemmas, tags, labels, parents, sentId=None):
        ...
        if strength == '~':  # includes transitive closure over all member strong groups
            ...
            if offset in i2sgroup:  # include strong group of child
                for o in sgroups[i2sgroup[offset]]:
                    i2wgroup[o] = i2wgroup[
                        offset]  # in case the last word in a strong expression precedes part of a weak expression
                    g.append(o)
            else:
                g.append(offset)

    # sanity check: number of tokens belonging to some MWE
    assert len(set(sum(sgroups + wgroups, []))) == sum(1 for t in tags if t[0].upper() != 'O'), (
    sentId, tags, sgroups, wgroups)
    ...
    for k, lbl in enumerate(labels):
        if lbl:
            assert not any(k + 1 in g and k + 1 != g[0] for g in
                           sgroups), 'Label for a strong group must only be present for its first token: ' + lbl + ' at ' + str(
                k + 1)
